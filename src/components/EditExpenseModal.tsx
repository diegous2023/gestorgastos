...
import { Expense, CATEGORIES, CategoryId, Currency } from '@/types/expense';
...
const EditExpenseModal: React.FC<EditExpenseModalProps> = ({ expense, isOpen, onClose }) => {
  const { updateExpense, customCategories } = useExpenses();
  ...
  useEffect(() => {
    if (expense) {
      ...
      setAmountUSD(expense.amountUSD?.toString() || '');
